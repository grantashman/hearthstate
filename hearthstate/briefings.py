...
def build_briefing(store: PlannerStore, viewer: str, now: datetime | None = None) -> str | None:
    ...
    if current.hour < 7 or current.hour >= 21:
        return None
    if store.briefing_claimed(viewer, "morning", current.date().isoformat()):
        return None
...
def main() -> None:
    ...
    try:
        message = build_briefing(store, args.viewer)
        if message:
            print(message)
            claim_briefing(store, args.viewer, "morning")
    finally:
        store.close()
