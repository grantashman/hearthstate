...
- Hosted onboarding is self-serve for authenticated accounts without a membership: `/setup` calls the transactional `create_household` RPC and redirects existing members to `/`. Owner-confirmed household export/deletion is implemented through the hosted portability boundary.
- Meal creates, edits, and deletion use transactional `create_meal`/`update_meal`/`delete_meal` RPCs: actor and relevant rows are locked, cook UUIDs must belong to the active household, clearing the cook is supported, and activity history records creation, before/after edits, and deletion before-state. Direct authenticated PostgREST meal inserts, updates, and deletes are revoked.
- The grocery matcher now mutates household prices only during an explicit retailer refresh; ordinary reads are side-effect free, and recommendations require equivalent product sizes/variants. Controlled aliases support retailer-specific naming, size-qualified recipe lines become one packaged purchase only when a catalog product supports that interpretation, and an opt-in closest-pack fallback is displayed as non-equivalent planning information. The supported comparison surface is Coles and Woolworths only. The dashboard exposes both store prices per item, separate cart totals, per-item savings, live/curated provenance, and stale cached quote labels. Price/provenance fields are server-controlled: ordinary grocery writes cannot set them, the manual-price route supplies fixed manual metadata, and the live provider contract validates HTTPS retailer URLs, comparison keys, rate limits, freshness, and household-scoped quote persistence.
- Live retailer refresh is implemented behind an approved HTTPS provider configured with `HEARTHSTATE_LIVE_RETAILER_URL` and optional `HEARTHSTATE_LIVE_RETAILER_API_KEY`; direct retailer-page scraping is not part of the hosted runtime. Apply `20260806010000_coles_woolworths_live_quotes.sql` before deploying the API so quote metadata and the narrowed retailer constraint are available. Without provider configuration, the feature fails closed to curated Coles/Woolworths observations.
- Inbox captures are universal across the dashboard and Photon bridge. New captures are created with a pending `inbox_suggestions` row; review accept/edit/reject is transactional and locks membership, suggestion, and capture together. Inbox snapshots use an actor-bound security-definer read RPC, so Photon service-role state cannot bypass private-capture visibility. Private captures can only be reviewed by their creator, and activity records store decision facts without raw capture text.
