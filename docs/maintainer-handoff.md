...
- Meal edits use the transactional `update_meal` RPC: actor and meal rows are locked, cook UUIDs must belong to the active household, clearing the cook is supported, and before/after state is appended to activity history. Direct authenticated PostgREST meal updates are revoked.
